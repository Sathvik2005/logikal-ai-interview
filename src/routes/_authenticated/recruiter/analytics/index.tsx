...
function Analytics() {
  ...
  const completions = useQuery({
    queryKey: ["a", "completions"],
    queryFn: () => completionsFn({}),
  });
  ...
  const avgDays = tthRows.length
    ? Math.round(
        (tthRows.reduce((a, r) => a + Number(r.days_to_hire ?? 0), 0) / tthRows.length) * 10,
      ) / 10
    : null;

  const completed = (completions.data ?? []).reduce(
    (a, r) => a + (r.status === "completed" ? Number(r.total) : 0),
    0,
  );
  ...
  const avgIntegrity =
    integSummary?.avg_integrity != null ? Math.round(Number(integSummary.avg_integrity)) : null;
  ...
  function exportAll() {
    const blob = [
      "# Completion rates",
      toCsv(completions.data ?? []),
      "",
      "# Hiring trends",
      toCsv(trendRows as never),
      "",
      "# Time to hire",
      toCsv(tthRows as never),
      "",
      "# Persona effectiveness",
      toCsv((personas.data ?? []) as never),
      "",
    ].join("\n");
    ...
    <>
      <div className="mb-lg flex flex-wrap items-start justify-between gap-md">
        <div>
          ...
          <p className="text-body-lg text-on-surface-variant">
            Operational and quality metrics across your hiring funnel.
          </p>
        </div>
        <div className="flex gap-sm">
          <button
            onClick={refresh}
            className="px-3 py-2 border border-outline-variant rounded-lg inline-flex items-center gap-2 hover:bg-surface-container-low"
          >
            <Icon name="refresh" />
            Refresh
          </button>
          <button
            onClick={exportAll}
            className="px-3 py-2 bg-primary text-on-primary rounded-lg inline-flex items-center gap-2 hover:brightness-110"
          >
            <Icon name="download" />
            Export CSV
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-md mb-lg">
        <CardShadow className="p-lg">
          ...
          <p className="text-headline-lg mt-1">
            {tth.isLoading ? "…" : avgDays != null ? `${avgDays} days` : "—"}
          </p>
        ...
        <CardShadow className="p-lg">
          ...
          <p className="text-headline-lg mt-1">
            {personas.isLoading ? "…" : (personas.data ?? []).length}
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-lg">
        <CardShadow className="lg:col-span-2 p-lg">
          ...
          {funnel.isLoading ? (
            ...
            <div className="space-y-md">
              ...
              {funnelTotal <= 1 && (
                <p className="text-body-md text-on-surface-variant">No interview activity yet.</p>
              )}
            </div>
          )}
        </CardShadow>
              ...
              {(personas.data ?? []).slice(0, 6).map((p) => (
                <li key={p.persona_id} className="flex justify-between gap-2">
                  ...
                  <span className="text-on-surface-variant font-data-mono">
                    {p.completed_count} •{" "}
                    {p.avg_score != null ? Math.round(Number(p.avg_score)) : "—"}
                  </span>
                </li>
              ))}
              {(personas.data ?? []).length === 0 && (
                <li className="text-on-surface-variant">No persona stats yet.</li>
              )}
            </ul>
          )}
        </CardShadow>
        ...
        <CardShadow className="lg:col-span-3 p-lg">
                ...
                return (
                  ...
                  >
                    <div className="relative w-full h-32 flex items-end">
                      <div
                        className="absolute bottom-0 left-0 w-full bg-primary-container rounded-t"
                        style={{ height: `${h}%` }}
                      />
                      <div
                        className="absolute bottom-0 left-0 w-full bg-primary rounded-t"
                        style={{ height: `${hc}%` }}
                      />
                    </div>
                    <span className="text-[10px] text-on-surface-variant">
                      {String(r.week).slice(5)}
                    </span>
                  </div>
                );
              })}
          ...
          <div className="flex gap-md mt-2 text-label-caps uppercase text-on-surface-variant">
            <span className="inline-flex items-center gap-1">
              <span className="w-3 h-3 bg-primary-container rounded" />
              Scheduled
            </span>
            <span className="inline-flex items-center gap-1">
        ...
        <CardShadow className="lg:col-span-3 p-lg ai-insight">
          <h3 className="text-headline-sm flex items-center gap-2 mb-2">
