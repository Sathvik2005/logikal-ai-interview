          ...
          {jobs.map((j) => (
            <CardShadow key={j.id} className="p-lg flex flex-col">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-label-caps uppercase ${STATUS_TONE[j.status]}`}
                >
                  {j.status}
                </span>
              </div>
              ...
              <p className="text-data-mono text-on-surface-variant mt-md">
                {j.candidateCount} candidates matched
              </p>
              <div className="mt-auto pt-lg flex gap-sm flex-wrap">
                ...
                <button
                  type="button"
                  onClick={() => setImportJobId(j.id)}
                  className="flex-1 text-center px-3 py-2 border border-outline-variant rounded-lg hover:bg-surface-container-low inline-flex items-center justify-center gap-1"
                >
                  <Icon name="upload" />
                  Bulk import
                </button>
