      ...
      <div className="mb-lg grid grid-cols-[minmax(0,1fr)_auto] items-center gap-md sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-body-lg text-on-surface-variant">
            {live
              ? `${live.candidateName} • ${live.role} • ${live.personaName}`
      ...
      {liveQ.isLoading ? (
        ...
        <CardShadow className="p-lg">
          <EmptyState
            ...
            hint="Scheduled sessions will appear here when they start."
          />
        </CardShadow>
      )}
        ...
        <div className="mb-md flex items-end justify-between flex-wrap gap-md">
          <div>
            ...
            <p className="text-body-md text-on-surface-variant">
              Every completed interview is recorded. Click any tile to replay the session.
            </p>
          </div>
          <span className="text-label-caps uppercase text-on-surface-variant">
            {recorded.length} recordings
          </span>
        </div>
        ...
        {recordedQ.isLoading ? (
          ...
          <CardShadow className="p-lg">
            <EmptyState
              ...
              hint="Recordings will appear after candidates complete an interview."
            />
          </CardShadow>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-lg">
                ...
                >
                  <CardShadow className="overflow-hidden transition hover:shadow-lg hover:-translate-y-0.5">
                    <div className="aspect-video bg-inverse-surface relative flex items-center justify-center">
                      <div className="w-20 h-20 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center font-bold text-headline-md">
                        {initials}
                      </div>
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 bg-black/30 transition">
                        <div className="w-14 h-14 rounded-full bg-primary text-on-primary flex items-center justify-center">
...
function LiveSession({ live }: { live: LiveInterviewDTO }) {
  ...
  const sessionId =
    (data as { session?: { id: string } | null } | undefined)?.session?.id ?? live.sessionId;
        ...
        <div className="lg:col-span-2 space-y-lg">
          <CardShadow className="aspect-video bg-inverse-surface text-inverse-on-surface relative overflow-hidden">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-32 h-32 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center text-on-primary-container text-display-lg font-bold">
                {initials}
              </div>
            </div>
            <div className="absolute bottom-md left-md right-md flex items-center justify-between text-label-caps uppercase">
              <span
                className={`px-2 py-1 rounded ${observing ? "bg-error/90" : "bg-on-surface-variant/80"}`}
              >
                {observing ? "LIVE" : "PAUSED"}
              </span>
          ...
          <CardShadow>
            <div className="p-lg border-b border-outline-variant">
              <h3 className="text-headline-sm">Live transcript</h3>
            </div>
            <ul className="p-lg space-y-md max-h-[320px] overflow-y-auto">
              {turns.length === 0 && (
                <li className="text-on-surface-variant text-body-md italic">
              ...
              {turns.map((m) => (
                <li key={m.id} className={`flex gap-3 ${m.who === "AI" ? "" : "flex-row-reverse"}`}>
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-label-caps font-semibold ${m.who === "AI" ? "bg-primary-container text-on-primary-container" : "bg-secondary-container text-on-secondary-container"}`}
                  >
                    {m.who === "AI" ? "AI" : "C"}
                  </div>
                  <div
                    className={`max-w-[80%] p-3 rounded-lg ${m.who === "AI" ? "bg-surface-container-low" : "bg-primary-fixed text-on-primary-fixed-variant"}`}
                  >
                    {m.text}
                  </div>
                </li>
        ...
        <div className="space-y-lg">
          <CardShadow className="p-lg">
            ...
            <div className="flex flex-col gap-2">
              ...
              {error && (
                <p className="text-body-sm text-error">Connection error — try reconnect.</p>
              )}
              {!observing && (
                <p className="text-body-sm text-on-surface-variant italic">Monitoring paused.</p>
              )}
            </div>
          </CardShadow>
          ...
          <CardShadow className="p-lg">
            ...
            <dl className="text-body-md space-y-2">
              <div className="flex justify-between">
                <dt className="text-on-surface-variant">Persona</dt>
                <dd>{live.personaName}</dd>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <dd>{live.durationMinutes} min</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-on-surface-variant">Status</dt>
                <dd className="capitalize">{live.status.replace(/_/g, " ")}</dd>
              </div>
            </dl>
          </CardShadow>
          ...
          <CardShadow className="p-lg ai-insight">
            <h3 className="text-headline-sm mb-4 flex items-center gap-2">
              ...
              Live Signals & Proctoring
            </h3>
            {events.length === 0 ? (
              <p className="text-body-md text-on-surface-variant italic">
                No alerts or events recorded yet.
              </p>
            ) : (
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {events.map((ev) => {
                  ...
                  const isFlag =
                    ev.type === "manual_flag" ||
                    ev.type.includes("lost") ||
                    ev.type.includes("switch");
                  return (
                    <div
                      key={ev.id}
                      className={`p-3 rounded-lg border text-body-md flex items-start gap-2 ${isFlag ? "bg-error-container/20 border-error/20 text-on-error-container" : "bg-surface-container-low border-outline-variant"}`}
                    >
                      <Icon
                        name={isFlag ? "warning" : "info"}
                        className={`text-md ${isFlag ? "text-error" : "text-primary"}`}
                      />
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold capitalize">{ev.type.replace(/_/g, " ")}</p>
                        {note && (
                          <p className="text-body-sm mt-0.5 text-red-700 font-medium">{note}</p>
                        )}
        ...
        >
          ...
          >
            ...
            <p className="text-body-md text-on-surface-variant mb-md">
              The candidate will be disconnected and the AI evaluation will be queued. This cannot
              be undone.
            </p>
            <div className="flex justify-end gap-2">
              ...
              <button
                type="button"
                onClick={handleEnd}
                className="px-3 py-2 rounded-lg bg-error text-on-error font-semibold"
              >
                End session
              </button>
            </div>
...
function PlaybackModal({
  ...
  return (
    ...
    >
      <div
        className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-soft w-full max-w-4xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-md border-b border-outline-variant flex items-center justify-between">
          <div>
            ...
            <p className="text-label-caps uppercase text-on-surface-variant">
              {recorded.role} • {recorded.personaName} •{" "}
              {recorded.scheduledAt ? fmtDate(recorded.scheduledAt) : "—"}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-0">
          <div className="md:col-span-3 bg-inverse-surface relative aspect-video flex items-center justify-center">
            <div className="w-28 h-28 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center font-bold text-display-sm">
              {initials}
            </div>
            <span className="absolute bottom-2 right-2 text-label-caps bg-black/60 text-white px-2 py-0.5 rounded">
          ...
          <div className="md:col-span-2 border-l border-outline-variant flex flex-col max-h-[60vh]">
            <div className="p-md border-b border-outline-variant text-label-caps uppercase text-on-surface-variant">
              Transcript
            </div>
            <ul ref={transcriptRef} className="flex-1 overflow-y-auto p-md space-y-2">
              {isLoading && (
                <li className="text-on-surface-variant italic text-body-md">Loading transcript…</li>
              )}
              {!isLoading && turnsWithT.length === 0 && (
                <li className="text-on-surface-variant italic text-body-md">
              ...
              {visible.map((turn, i) => (
                ...
                >
                  <span className="text-label-caps uppercase opacity-70 mr-2">
                    {formatTime(turn.t)} · {turn.who}
                  </span>
                  {turn.text}
                </li>
              ))}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center hover:brightness-110"
              ...
              <button
                ...
                className="w-10 h-10 rounded-full border border-outline-variant flex items-center justify-center hover:bg-surface-container-low"
                aria-label="Restart"
              >
                <Icon name="replay" />
              </button>
              <span className="text-label-caps uppercase text-on-surface-variant ml-2">
                {formatTime(current)} / {formatTime(durationSec)}
              </span>
            </div>
            <Link
              to="/recruiter/reports/$interviewId"
              params={{ interviewId: recorded.interviewId }}
              className="px-3 py-2 bg-secondary text-on-secondary rounded-lg flex items-center gap-2 hover:brightness-110 text-body-md"
            >
