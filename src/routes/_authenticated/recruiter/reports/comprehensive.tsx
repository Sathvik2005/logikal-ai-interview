  ...
  const stats = useMemo(
    () => ({
      ...
      avgScore: scored.length
        ...
        : null,
      avgInteg: integrityScored.length
        ? Math.round(
    ...
    <>
      <div className="mb-lg flex items-start justify-between gap-md flex-wrap">
        <div>
          <Link
            ...
            className="text-primary text-body-md hover:underline inline-flex items-center gap-1 mb-2"
          >
          ...
          <p className="text-body-lg text-on-surface-variant">
            Cross-candidate ranking and breakdown for this hiring cycle.
          </p>
        </div>
      </div>
      ...
      {completed.length === 0 ? (
        <EmptyState
          ...
          hint="The ranking table will populate once candidates complete their interviews and evaluations are generated."
          className="py-20"
        />
      ) : (
        <>
              ...
              <table className="w-full text-body-md" style={{ minWidth: "700px" }}>
                <thead className="bg-surface-container-low text-label-caps uppercase text-on-surface-variant">
                  <tr>
                    ...
                    <th className="text-left p-4 border-b border-outline-variant">
                      Recommendation
                    </th>
                ...
                <tbody className="divide-y divide-outline-variant/60">
                  {ranked.map((r, i) => (
                    <tr key={r.id} className="hover:bg-surface-container-low/70 transition">
                      <td className="p-4">
                        <span
                          className={`font-bold text-headline-sm ${i === 0 ? "text-[#854d0e]" : i === 1 ? "text-on-surface-variant" : i === 2 ? "text-[#7e3000]" : "text-on-surface-variant"}`}
                        >
                          #{i + 1}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        {r.recommendation ? (
                          <span
                            className={`px-2.5 py-1 rounded-full text-label-caps font-semibold ${
                              r.recommendation === "strong_hire"
                                ? "badge-strong-hire"
