    ...
    <div className="space-y-lg">
      <header>
        ...
        <p className="text-body-lg text-on-surface-variant">
          Error events and admin audit history from the last batch.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-md">
        {[
          ...
          {
            label: "Error sources",
            value: new Set(errors.map((e) => e.source ?? "unknown")).size,
            icon: "category",
            tone: "default",
          },
        ].map((k) => (
        ...
        <Card className="p-lg">
            ...
            <div className="flex gap-1">
              {(["all", "info", "warn", "error", "fatal"] as const).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => setLvl(s)}
                  className={`px-2 py-1 rounded-full text-label-caps capitalize ${lvl === s ? "bg-primary text-on-primary" : "bg-surface-container text-on-surface-variant hover:bg-surface-container-high"}`}
                >
                  {s}
                </button>
              ))}
          ...
          {errQ.isLoading ? (
            ...
            <EmptyState
              icon="security"
              title="No events"
              description="No error events match this level."
            />
          ) : (
              ...
              {filtered.map((e) => (
                <li key={e.id} className="p-3 rounded-lg border border-outline-variant">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`text-label-caps font-semibold px-2 py-1 rounded-full capitalize ${LEVEL_TONE[e.level ?? "info"] ?? LEVEL_TONE.info}`}
                    >
                      {e.level ?? "info"}
                    </span>
                  </div>
