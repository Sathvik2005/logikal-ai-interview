...
import {
  ...
  type CandidateInterviewSummary,
} from "@/lib/candidate-self.functions";
        ...
        {!list.isLoading && upcoming.length === 0 && (
          <Card className="p-0 overflow-hidden">
            <EmptyState
              ...
              description="You'll get an email with a join link when one is scheduled."
            />
          </Card>
        )}
        {upcoming.map((iv) => (
      ...
      <div>
        ...
        <p className="text-body-sm text-on-surface-variant">
          {when
            ...
            : "Flexible start"}{" "}
          · {iv.durationMinutes} min · {iv.personaName}
        </p>
        {countdown && <p className="text-body-sm text-primary mt-1">Starts in {countdown}</p>}
      </div>
