...
export const getRecruiterDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: cands }, { data: ivs }, { data: tops }] = await Promise.all([
      context.supabase.from("candidates").select("id, status").is("deleted_at", null),
      context.supabase
        .from("interviews")
    ...
    const liveOrScheduledCount = (ivs ?? []).filter(
      (i: { status: string }) => i.status === "scheduled" || i.status === "in_progress",
    ).length;
    ...
    const todaysInterviews: DashboardInterview[] = (ivs ?? [])
      .filter(
        (i: { scheduled_at: string | null }) =>
          i.scheduled_at &&
          new Date(i.scheduled_at) >= today &&
          new Date(i.scheduled_at) < tomorrow,
      )
      .map(
        (i: {
          id: string;
    ...
    const topCandidates: DashboardCandidate[] = (
      (tops ?? []) as {
        id: string;
        full_name: string;
