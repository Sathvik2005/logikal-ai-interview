    ...
    <div className={`stat-card flex flex-col justify-between min-h-[120px] ${className}`}>
      <div>
        <div className="flex items-start justify-between mb-2">
          <p className="text-label-caps uppercase text-on-surface-variant tracking-wider leading-none">
            {label}
          </p>
          {icon && (
            <div
        ...
        <div className="text-headline-lg font-bold text-on-background leading-none mt-1">
          {value}
        </div>
      </div>
      {(hint || trend) && (
        <div className="flex items-center justify-between mt-3 pt-2 border-t border-outline-variant/30">
          ...
          {trend && (
            <span
              className={`text-label-caps font-semibold ${trend.value >= 0 ? "text-[#166534]" : "text-error"}`}
            >
