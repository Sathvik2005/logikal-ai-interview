...
export function QuestionBankWizard({
    ...
    mutationFn: () =>
      saveFn({
        data: {
          ...
          questions: items
            .filter((i) => i.prompt.trim().length >= 4)
  ...
  function close() {
    ...
    setTimeout(() => {
      ...
      setMode("manual");
      setBankName("");
      setCompetency(defaultCompetency);
      setItems([blankQ()]);
    }, 200);
  }
  ...
  async function onPdf(f: File) {
    ...
    try {
      ...
      if (!res.questions.length) {
        ...
        const draft: DraftQ[] = res.questions.map((q) => ({
          id: uid(),
          prompt: q.prompt,
        ...
        setItems((cur) =>
          mode === "hybrid" ? [...cur.filter((c) => c.prompt.trim()), ...draft] : draft,
        );
  ...
  const canSave =
    ...
    competency.trim().length >= 2 &&
    items.some((i) => i.prompt.trim().length >= 4);
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 bg-white">
              ...
              {[
                {
                  ...
                  icon: "edit_note",
                  title: "Manual entry",
                  hint: "Write each question yourself",
                },
                {
                  id: "pdf" as Mode,
                  icon: "picture_as_pdf",
                  title: "PDF upload",
                  hint: "Extract from an existing document",
                },
                {
                  id: "hybrid" as Mode,
                  icon: "merge_type",
                  title: "Hybrid",
                  hint: "Start from PDF, then refine manually",
                },
              ].map((m) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left p-4 rounded-xl border transition ${selected ? "border-primary bg-primary-fixed/60 ring-2 ring-primary-container/40" : "border-outline-variant hover:bg-surface-container-low"}`}
                  >
                    <span
                      className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${selected ? "bg-primary text-on-primary" : "bg-surface-container text-on-surface-variant"}`}
                    >
                      <Icon name={m.icon} />
                    </span>
          ...
          {step === 2 && (
            <div>
                ...
                <div className="mb-4 p-4 border border-dashed border-outline-variant rounded-xl bg-surface-container-lowest flex items-center justify-between flex-wrap gap-3">
                  <div>
                    ...
                    <p className="text-body-md text-on-surface-variant">
                      We'll extract questions and let you edit them below.
                    </p>
                  </div>
                  <div>
                    <input
                    ...
                    <button
                      ...
                      disabled={parsing}
                      className="px-4 py-2 bg-primary text-on-primary rounded-lg inline-flex items-center gap-2 disabled:opacity-60"
                    >
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex flex-col">
                        <button
                          onClick={() => move(q.id, -1)}
                          disabled={idx === 0}
                          className="p-1 text-on-surface-variant disabled:opacity-30"
                        >
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <label className="flex items-center gap-1 text-sm">
                            <input
                              type="checkbox"
                              checked={q.mandatory}
                              onChange={(e) => update(q.id, { mandatory: e.target.checked })}
                            />{" "}
                            Mandatory
                          </label>
                          <input
                            className="flex-1 min-w-[200px] px-2 py-1 border border-outline-variant rounded text-sm"
                            placeholder="Expected hints (comma separated)"
                            value={q.hints.join(", ")}
                            onChange={(e) =>
                              update(q.id, {
              ...
              <button
                onClick={() => setItems((xs) => [...xs, blankQ()])}
                className="mt-3 px-3 py-2 border border-dashed border-outline-variant rounded-lg text-body-md inline-flex items-center gap-2 hover:bg-surface-container-low"
              >
                <Icon name="add" />
                Add question
              </button>
          ...
          {step === 3 && (
            <div className="max-w-xl mx-auto text-center py-6">
              ...
              <p className="text-body-md text-on-surface-variant mb-4">
                {items.filter((i) => i.prompt.trim().length >= 4).length} question
                {items.length === 1 ? "" : "s"} will be added to competency{" "}
                <strong>{competency || "—"}</strong>.
              </p>
              <button
                onClick={() => save.mutate()}
                disabled={!canSave || save.isPending}
                className="px-5 py-2.5 bg-primary text-on-primary rounded-lg font-semibold inline-flex items-center gap-2 disabled:opacity-60"
              >
                <Icon name="save" />
                {save.isPending ? "Saving…" : "Save bank"}
              </button>
            </div>
          )}
        </div>
        ...
        <footer className="px-6 py-3 border-t border-outline-variant flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {step < 3 && (
              <button
                onClick={() => {
                  if (step === 1 && (bankName.trim().length < 2 || competency.trim().length < 2)) {
                    toast.error("Bank name and competency required");
                    return;
                  }
