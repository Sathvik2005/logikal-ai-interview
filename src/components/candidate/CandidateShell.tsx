  ...
  return (
    <div className="bg-background text-on-background min-h-screen flex">
      <aside className="hidden md:flex flex-col h-screen sticky left-0 top-0 w-64 bg-surface border-r border-outline-variant p-md">
        <div className="mb-lg">
          <Link to="/candidate/prepare" className="font-headline-md text-headline-md font-bold text-primary block">Logikality AI</Link>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex justify-between items-center h-16 px-lg w-full sticky top-0 z-40 bg-surface/80 backdrop-blur-md border-b border-outline-variant">
          <div className="flex items-center gap-2 md:hidden">
            <MobileNav items={NAV} rootPath="/candidate/prepare" brand="Logikality AI" subtitle="Candidate Portal" />
            <Link to="/candidate/prepare" className="text-headline-sm font-black text-on-surface">Logikality AI</Link>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-container-low">
              ...
              <span className="text-body-md text-on-surface hidden sm:block">{user?.email ?? "candidate"}</span>
            </div>
            <button
              type="button"
              onClick={signOut}
