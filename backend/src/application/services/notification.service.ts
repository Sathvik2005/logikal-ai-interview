...
export class NotificationService implements OnModuleInit {
  ...
  constructor(
    ...
    if (apiKey && apiKey !== 're_yourResendApiKeyHere' && apiKey.startsWith('re_')) {
      ...
      this.logger.warn('RESEND_API_KEY is missing or dev template. Emails will fall back to log printing.');
    }
  }
  ...
  async sendEmail(orgId: string, kind: string, recipientEmail: string, payload: any): Promise<void> {
    ...
    if (!this.resend) {
      this.logger.log(`[Email Output Mock LOG] To: ${recipientEmail} | Subject: "${subject}" | Content Preview: ${html.slice(0, 150)}...`);
