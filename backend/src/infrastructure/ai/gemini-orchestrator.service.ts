...
import { IAIOrchestrator, ParsedResume, SuggestedJd, JdMatchResult, EvaluationResult } from '../../application/common/ai/ai-orchestrator.interface';
...
export class GeminiOrchestratorService implements IAIOrchestrator {
  ...
  constructor(
    ...
    if (apiKey && apiKey !== 'your-gemini-api-key-here' && apiKey.length > 10) {
      ...
      this.logger.warn('GEMINI_API_KEY is missing or template key. AI Orchestrator running in Mock Fallback Mode.');
    }
  }
  ...
  async parseResume(resumeText: string): Promise<ParsedResume> {
    ...
    if (!this.hasClient()) {
      ...
      return {
        ...
        experienceYears,
        skills,
        resumeSummary: 'Experienced software developer with a strong background in web engineering and client-side applications.',
      };
    }
...
"requirements" should be a bulleted string listing core qualifications.`;
  ...
  async generateInterviewQuestion(
    ...
    try {
      const systemParts = [
        personaPrompt,
        `You are asking questions to the candidate.`,
      ];
      if (jdContext) systemParts.push(`Job description Context:\n${jdContext}`);
      if (candidateContext) systemParts.push(`Candidate Profile Context:\n${candidateContext}`);
      
      if (nextCuratedQuestion) {
        systemParts.push(`NEXT DIRECTIVE/QUESTION (incorporate or rephrase naturally in your interviewer voice): "${nextCuratedQuestion}"`);
      } else {
        systemParts.push('Curated questions lists are empty or complete. Switch to adaptive follow-up mode probing on previous responses.');
      }
      systemParts.push('Return ONLY the interviewer text. No markdown structure, no quotes, no JSON.');
  ...
  async evaluateInterview(
    ...
    if (!this.hasClient()) {
      ...
      return {
        overallScore: 82,
        recommendation: 'hire',
        strengths: ['Demonstrated solid modular programming practices', 'Clear communication style'],
        concerns: ['Slight hesitation when discussing asynchronous event handling loops'],
        summary: 'The candidate exhibited good core capability suitable for web applications. Integrity was clear with minor tabs switches.',
        competencyScores: {
          'Technical Skills': 85,
          'Communication': 80,
          'Problem Solving': 82,
